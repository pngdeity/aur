...
- [x] **Disposition of `manifest.json`**: Resolved 2026-05-15 swarm (S8.2):
      ...
      Regenerated by `validate-pkgbuilds-pkl.sh` at CI time. **(Pending:
      `git rm --cached manifest.json` — file remains tracked; `.gitignore` alone
      does not untrack.)**
- [x] **Disposition of `nvc_versions.json`**: Resolved — added to `.gitignore`
      as machine-local nvchecker state.
...
- [ ] **chroot builds — 27 packages**: Run `pkgctl build` for all packages in a
...
- [ ] **AUR deployment (SSH auth)**: Configure AUR SSH keys and test
      ...
      python-whois).
- [x] **nvc_versions.json decision**: Resolved — added to `.gitignore` as
      machine-local nvchecker state.
